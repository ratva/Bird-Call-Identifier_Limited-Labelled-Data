...
def train(audio_model, train_loader, val_loader, args):
    print("Summary writer initialized")
    log_dir = f'runs/{args.lr}_{args.lrscheduler_decay}_{args.last_layer_ft}'
    writer = SummaryWriter(log_dir=log_dir)
    writer.add_scalar('Hypers/Learning_Rate', args.lr, 0)
    writer.add_scalar('Hypers/Decay', args.lrscheduler_decay, 0)
    writer.add_scalar('Hypers/LP', args.last_layer_ft, 0)
    ...
    if args.last_layer_ft:
        for param in audio_model.parameters():
            param.requires_grad_(False)
        audio_model.module.mlp_head.requires_grad_(True)
    ...
    if args.last_layer_ft:
        mlp_head_lr = args.lr * args.head_lr
        optimizer = torch.optim.Adam(mlp_params, mlp_head_lr, weight_decay=5e-7, betas=(0.95, 0.999))
    else:
    ...
    # [
    #     {'params': [p for p in trainables if p in mlp_params], 'lr': args.lr * args.head_lr},
    #     {'params': [p for p in trainables if p not in mlp_params], 'lr': args.lr}
    # ],
    # weight_decay=5e-7,
    # betas=(0.95, 0.999))
    ...
    while epoch < args.n_epochs + 1:
        ...
        writer.add_scalar('AvgLoss/train', loss_meter.avg, epoch)
        ...
        writer.add_scalar("mAUC/val", mAUC, epoch)
        writer.add_scalar("d_prime", d_prime(mAUC), epoch)

        writer.add_scalar("avg_precision", average_precision, epoch)
        writer.add_scalar("avg_recall", average_recall, epoch)
        ...
        # logic for early stopping is based on validation cross entropy
        if valid_loss < best_va_loss:
            best_va_loss = valid_loss
            best_va_loss_epoch = epoch-1
            epochs_no_improvement = 0
        else:
            epochs_no_improvement += 1
        
        if epochs_no_improvement >= 8:
    ...
    # if args.wa == True:
    #     stats = validate_wa(audio_model, val_loader, args, args.wa_start, args.wa_end)
    #     mAP = np.mean([stat['AP'] for stat in stats])
    #     mAUC = np.mean([stat['auc'] for stat in stats])
    #     middle_ps = [stat['precisions'][int(len(stat['precisions'])/2)] for stat in stats]
    #     middle_rs = [stat['recalls'][int(len(stat['recalls'])/2)] for stat in stats]
